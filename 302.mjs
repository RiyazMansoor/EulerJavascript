...
function is_achilles( pow_num ) {

    const powers = Array.from( pow_num.values() ) ;
    ...
    // n != a^b  
    return ArrayGCD( powers ) === 1 ;

} 

function euler_totient( pow_num ) {

    const new_pow_num = new PowerNumber( pow_num ) ;

    for ( const base of Array.from( pow_num.keys() ) {

        new_pow_num.Map.set( base, new_pow_num.Map.get( base ) - 1 ) ;
    
        if ( base == 2 ) continue ;
        new_pow_num = new_pow_num.Multiply( new PowerNumber( base - 1 ) ) ;
...
function recurse( this_depth, this_value, this_num, this_primes, limit, factors ) {
...
function E302( limit ) {
    ...
    const numbers = new PrimeFactors( Math.trunc( Math.pow( limit/4, 0.34 ) ) ) ;
    const primes = numbers.PrimesSet ;

    const factors = {}, factor_next = {} ;
    for ( const prime of primes ) {
        factors[prime] = numbers.Factors( prime - 1 ) ;
        factor_next[prime] = {} ;
    }
