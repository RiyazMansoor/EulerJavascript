...
/*
function SieveCount( factors, length ) {

    const period = factors.reduce( ( total, value ) => total * value, 1 ) ;
    const period_cnt = Math.floor( length / period ), period_rem = length % period ;

    if ( factors.length == 1 ) {
        return length - ( period_cnt + ( period_rem > 0 ? 1 : 0 ) ) ;        
    }

    let match_cnt = 0 ;
    for ( const factor of factors ) {
        match_cnt += period / factor ;
        match_cnt += Math.floor( period_rem / factor ) ;
    }
