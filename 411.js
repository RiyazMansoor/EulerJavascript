...
var E411;
(function (E411) {
    const ORIGIN = { x: 0, y: 0, next: [], value: 1 };
    function coordToStr(p) {
        return `(${p.x},${p.y})[${p.value}]`;
    }
    function pointToStr(p) {
        const nextPoints = p.next.reduce((pv, cv) => `${pv}${coordToStr(cv)}|`, "");
    ...
    function stationPoints(N) {
        const points = [{ x: 1, y: 1, next: [], value: 1 }];
    ...
    function reachable(p1, p2) {
        return p2.x >= p1.x && p2.y >= p1.y;
    }
    function addPoint(searchPoint, newPoint) {
    ...
    function run() {
        ...
        const points = stationPoints(ARG2);
